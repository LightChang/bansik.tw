...
| `observation.csv` `.md` | 某時間點觀察到的值：放名額日、每月名額、預約方式、電話與分機、門診時間。`entity_key` 可 join `entity.csv` | 134 |
...
- **動態層目前只解析得到四個來源**：中山醫附醫（放名額日、每月名額）、新北衛生局（預約方式）、臺中衛生局（電話與分機）、嘉義市衛生局（電話與門診時間）。各聯評中心自家網頁還沒逐站解析，卡在「拿不到網址」——名錄裡只有 9/90 家填了官網。
