...
| `observation.csv` `.md` | 某時間點觀察到的值：放名額日、每月名額、預約方式、電話與分機、門診時間。`entity_key` 可 join `entity.csv` | 185，涵蓋 89 家機構 |
...
- **動態層目前解析得到五個來源**：國健署全國聯絡資訊（一頁涵蓋 22 縣市共 51 家的電話與分機）、中山醫附醫（放名額日、每月名額）、新北衛生局（預約方式）、臺中衛生局（電話與分機）、嘉義市衛生局（電話與門診時間）。
- **逐縣市翻衛生局頁的投報率很低**：查了 5 個縣市才拿到 3 個可解析的頁面，而國健署那份一頁就覆蓋全國。要擴充動態層，先找全國層級的彙整，再補縣市特有的欄位（例如嘉義市的門診時間、彰化的「核發哪種證明文件」）。
- **各聯評中心的網址已補了 17 個**（`scripts/urls.json`），來源是臺中與新北衛生局頁的連結欄。全庫有官網的從 68 家增為 **84 家**，聯合評估中心從 9/90 增為 **25/90**（新北 12 家全數補齊，臺中補了 5 家）。其中 12 個是 reurl.cc 短網址：**它不含任何轉址語法**（沒有 HTTP 3xx、沒有 meta refresh、沒有 `location.href`），body 裡只有 `og:title` 與 favicon，跳轉由 `/asset/javascripts/redirect/redirect.js` 執行後才發生——所以 curl 一律拿不到目標，剖 body 會誤抓成 favicon 的網址（亞東就是這樣被抓成 `googleusercontent`）。**只能用瀏覽器開啟、等 JS 跑完再讀 `location.href`**，每一筆都確認標題確實是該機構的聯評或早療單位。
- **不要憑網域判斷真偽**：恩主公醫院的聯評中心掛在 `eck6800.idv.tw`，看起來像個人網站，實際開啟確認就是該院自建的站。
- shorturl.at 的 5 個連結取不到（Cloudflare `cf-mitigated: challenge`，403），不繞，留空待人工補。
- **逐站解析放名額日仍未做**：有了網址只是前提，13 家抽查中只有 1 家公布放名額日，每家頁面模板又都不同。
- **縣市彙整頁的資訊密度差很多**：嘉義市是唯一有門診時間的，新北只有預約方式，臺中只有電話。桃園那頁抓不到（HiNetCDN 回 HTTP 428，防爬，不繞）。
- **等候天數沒有任何公開來源**：不只各中心不公布，縣市衛生局的彙整頁也只有聯絡方式。這是本題最想解決、但資料端補不上的一塊。
